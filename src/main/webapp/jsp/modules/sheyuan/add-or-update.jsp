...
<body>
					...
					<div class="row breadcrumb-bar">
						...
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>社员管理</span></li>
								<li class="breadcrumb-item active"><span>编辑社员</span></li>
							</ol>
						</div>
					</div>
				</div>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>社长账号</label>
										<select  class="form-control"  name="shezhangzhanghao" id="shezhangzhanghaoSelect" onchange="shezhangzhanghaoSelect2Change()">
										</select>
									</div>
									<div class="form-group col-md-6">
										<label>社团名称</label>
										<input id="shetuanmingcheng" name="shetuanmingcheng" class="form-control"  value="" readonly>
									</div>
	...
	<script>
		...
		function shezhangzhanghaoSelect2(){
			http("option/shezhang/shezhangzhanghao","GET",{
				tableName: "shezhang",
				columnName: "shezhangzhanghao"
			},(res)=>{
				if(res.code == 0){
					var options = res.data;
					var option = document.createElement("option");
					$("#shezhangzhanghaoSelect").append(option)
					for(var i=0;i<options.length;i++){
						var shezhangzhanghaoOption = document.createElement("option");
						shezhangzhanghaoOption.setAttribute("value",options[i]);
						shezhangzhanghaoOption.setAttribute("class","shezhangzhanghaoOption");
						if(ruleForm.shezhangzhanghao == options[i]){
							shezhangzhanghaoOption.setAttribute("selected","selected");
						}
						shezhangzhanghaoOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.shezhangzhanghao != null && ruleForm.shezhangzhanghao != '' && options[i] == ruleForm.shezhangzhanghao){
							shezhangzhanghaoOption.setAttribute("selected","selected");
						}
						$("#shezhangzhanghaoSelect").append(shezhangzhanghaoOption);
					}
				}
			});
		}		
		function shezhangzhanghaoSelect2Change(){
			var options = document.getElementById("shezhangzhanghaoSelect").options;
			var index = document.getElementById("shezhangzhanghaoSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/shezhang/shezhangzhanghao","GET",{
				tableName: "shezhang",
				columnName: "shezhangzhanghao",
				columnValue: colVal
			},(res)=>{
				if(res.code == 0){
					if(res.data != null && res.data.shetuanmingcheng != null){
		...
		function submit() {
                //获取表单数据前取消禁用
                $("#shezhangzhanghaoSelect").attr('disabled',false);
				...
				if(crossrefid && crossuserid) {
					...
					http("sheyuan/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("sheyuan/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#shezhangzhanghaoSelect").attr('disabled',true);
                $("#shetuanmingcheng").attr('readonly','readonly');
            }
		}
