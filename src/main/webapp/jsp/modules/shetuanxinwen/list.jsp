...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">社团新闻管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>社团新闻管理</span></li>
								<li class="breadcrumb-item active"><span>社团新闻列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">社团新闻列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<label>发布时间</label>
									<input id="fabushijian_start" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
									至
									<input id="fabushijian_end" class="Wdate form-control form-control-sm" type="text" onClick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd HH:mm:ss'})">
	...
	<script>
		...
		function search(){
			...
			if($('#fabushijian_start').val() != null && $('#fabushijian_start').val() != ''){
				searchForm.fabushijian_start = $('#fabushijian_start').val();
			}
			if($('#fabushijian_end').val() != null && $('#fabushijian_end').val() != ''){
				searchForm.fabushijian_end = $('#fabushijian_end').val();
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("shetuanxinwen/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				biaoti : searchForm.biaoti,	
				fabushijian_start : searchForm.fabushijian_start,
				fabushijian_end : searchForm.fabushijian_end,
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("shetuanxinwen/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("shetuanxinwen/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("shetuanxinwen/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function getSum(colName){
		http("shetuanxinwen"+colName,"GET",{
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"shetuanxinwen");
